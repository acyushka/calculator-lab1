...
Переходим в проект и открываем терминал:
- калькулятором уже можно пользоваться, но для запуска тестов нужно виртуальное окружение
...
У меня используется стандартное окружение

#### Для Windows

Создание .venv:

(Примечание: если не работает с python, пишем py)
...
(На всякий случай заново откройте терминал, чтобы находиться в виртуальном окружении)
